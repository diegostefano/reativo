...
extern uint8 can_msg_tx[8], received, is_positive[2];
extern union Data goal_dist[2], closest_obst[2], action[2];
/* `#END` */
    ...
    uint8 CAN_SendMsg0(void) 
    {
        uint8 result = CYRET_SUCCESS;
            ...
            else
            {
                ...
                CAN_TX_DATA_BYTE1(0) = goal_dist[0].bytes[0];
                CAN_TX_DATA_BYTE2(0) = goal_dist[0].bytes[1];
                CAN_TX_DATA_BYTE3(0) = goal_dist[0].bytes[2];
                CAN_TX_DATA_BYTE4(0) = goal_dist[0].bytes[3];
                CAN_TX_DATA_BYTE5(0) = goal_dist[1].bytes[0];
                CAN_TX_DATA_BYTE6(0) = goal_dist[1].bytes[1];
                CAN_TX_DATA_BYTE7(0) = goal_dist[1].bytes[2];
                CAN_TX_DATA_BYTE8(0) = goal_dist[1].bytes[3];
                
                /*
                CAN_TX_DATA_BYTE1(0) = can_msg_tx[0];
                CAN_TX_DATA_BYTE2(0) = can_msg_tx[1];
                CAN_TX_DATA_BYTE3(0) = can_msg_tx[2];
                CAN_TX_DATA_BYTE4(0) = can_msg_tx[3];
                CAN_TX_DATA_BYTE5(0) = can_msg_tx[4];
                CAN_TX_DATA_BYTE6(0) = can_msg_tx[5];
                CAN_TX_DATA_BYTE7(0) = can_msg_tx[6];
                CAN_TX_DATA_BYTE8(0) = can_msg_tx[7];
                */
                /* `#END` */
    ...
    uint8 CAN_SendMsg1(void) 
    {
        uint8 result = CYRET_SUCCESS;
            ...
            else
            {
                ...
                CAN_TX_DATA_BYTE1(1) = closest_obst[0].bytes[0];
                CAN_TX_DATA_BYTE2(1) = closest_obst[0].bytes[1];
                CAN_TX_DATA_BYTE3(1) = closest_obst[0].bytes[2];
                CAN_TX_DATA_BYTE4(1) = closest_obst[0].bytes[3];
                CAN_TX_DATA_BYTE5(1) = closest_obst[1].bytes[0];
                CAN_TX_DATA_BYTE6(1) = closest_obst[1].bytes[1];
                CAN_TX_DATA_BYTE7(1) = closest_obst[1].bytes[2];
                CAN_TX_DATA_BYTE8(1) = closest_obst[1].bytes[3];
                
                /*
                CAN_TX_DATA_BYTE1(1) = can_msg_tx[0];
                CAN_TX_DATA_BYTE2(1) = can_msg_tx[1];
                CAN_TX_DATA_BYTE3(1) = can_msg_tx[2];
                CAN_TX_DATA_BYTE4(1) = can_msg_tx[3];
                CAN_TX_DATA_BYTE5(1) = can_msg_tx[4];
                CAN_TX_DATA_BYTE6(1) = can_msg_tx[5];
                CAN_TX_DATA_BYTE7(1) = can_msg_tx[6];
                CAN_TX_DATA_BYTE8(1) = can_msg_tx[7];
                */
                /* `#END` */
    ...
    *******************************************************************************/
    void CAN_ReceiveMsg0(void) 
    {
        ...
        for (unsigned int i = 0; i < 4; i++) {
            action[0].bytes[i] = CAN_RX_DATA_BYTE(0, i);
            action[1].bytes[i] = CAN_RX_DATA_BYTE(0, i + 4);
        }
        led_Write(~led_Read());
        is_positive[0] = (action[0].word > 0.0f ? 1 : 0);
        is_positive[1] = (action[1].word > 0.0f ? 1 : 0);
        received = 1;
        /* `#END` */
